...
for d in dimensions:
    ...
    for size in sizes:
        ...
        parzen.append(testing_parzen.testing(d,size,0.5,1))
        #mle.append(testing_mle.testing(d,size,0.5))
    plt.plot(sizes,parzen,label=str(d)+"D",color=colors[i])
    #plt.plot(sizes,mle,label=str(d)+"D",color=colors[i])
...
#testing by trainning sampling size and variance of data
dimensions = [10,15,25]
size = 1500
...
for d in dimensions:
    ...
    for prob in probs:
        ...
        for t in range(10):
            ...
            parzen_errors_3.append(testing_parzen.testing(train_samples_1,test_samples_1,train_samples_2,test_samples_2,5))
    ...
    plotting.plot(title, parzen_3, mle,xtitle,ytitle,x_attr,"h = 5")
